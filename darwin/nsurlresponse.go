...
func NewNSURLResponse() INSURLResponse {
	r1 := nSURLResponseImportAPI().SysCallN(0)
	return AsNSURLResponse(r1)
}

func NewNSURLResponse1(aData NSURLResponse) INSURLResponse {
	r1 := nSURLResponseImportAPI().SysCallN(1, uintptr(aData))
	return AsNSURLResponse(r1)
}

func (m *TNSURLResponse) Data() NSURLResponse {
	r1 := nSURLResponseImportAPI().SysCallN(2, m.Instance())
...
func (m *TNSURLResponse) InitWithURLMIMETypeExpectedContentLengthTextEncodingName(uRL NSURL, mIMEType string, length int32, name string) INSURLResponse {
	r1 := nSURLResponseImportAPI().SysCallN(4, m.Instance(), uintptr(uRL), PascalStr(mIMEType), uintptr(length), PascalStr(name))
	return AsNSURLResponse(r1)
}
...
func (m *TNSURLResponse) MIMEType() string {
	r1 := nSURLResponseImportAPI().SysCallN(5, m.Instance())
	return GoStr(r1)
}

func (m *TNSURLResponse) ExpectedContentLength() (resultInt64 int64) {
	nSURLResponseImportAPI().SysCallN(3, m.Instance(), uintptr(unsafePointer(&resultInt64)))
	return
}
...
var (
	...
	nSURLResponseImportTables                  = []*imports.Table{
		...
		/*1*/ imports.NewTable("NSURLResponse_Create1", 0),
		/*2*/ imports.NewTable("NSURLResponse_Data", 0),
		/*3*/ imports.NewTable("NSURLResponse_ExpectedContentLength", 0),
		/*4*/ imports.NewTable("NSURLResponse_InitWithURLMIMETypeExpectedContentLengthTextEncodingName", 0),
		/*5*/ imports.NewTable("NSURLResponse_MIMEType", 0),
